...
public class Test implements Searchable {
    ...
    private void getTypeTransitions(List<Room> path) {
        int i = 0;
        Type type;
        int currentD = 0;
        ...
        for (Room room : path) {
            ...
            for (Transition transition : room.getTransitions()) {
                Transition trans = room.getTransitions().stream().filter(x -> x.getTransitionType().equals(Type.LIFT)).findFirst().orElse(null);
                if (trans != null && transition.getTransitionType().equals(Type.CLIMB)) {
                    continue;
                }

                if (transition.getToRoomNumber() == path.get(i + 1).getRoomNumber()) {
                    if (transition.getTransitionType().equals(Type.CLIMB)) {
                        currentD += (transition.getLength() * 2);
                    } else {
                        currentD += transition.getLength();
                    }
